...
import 'package:colorlive/customwidgets/searchbar_delegate.dart';
import 'package:colorlive/models/enums/media_type.dart';
import 'package:colorlive/models/enums/time_window.dart';
...
Future _onInit(Action action, Context<HomePageState> ctx) async {
  ...
  ctx.state.animatedController = AnimationController(vsync: ticker, duration: Duration(milliseconds: 600));
  ...
  if (p != null) {
    ctx.dispatch(HomePageActionCreator.onInitPopularMovie(p));
  }
  final t = await ApiHelper.getPopularTVShows();
  if (t != null) {
    ctx.dispatch(HomePageActionCreator.onInitPopularTV(t));
  }
}
